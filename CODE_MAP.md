...
This document maps each project capability to its main implementation entry points. The repository is an unofficial **π0.7-inspired research prototype** built on the public OpenPI π0.5 PyTorch Flow Matching path.
...
flowchart TD
    ...
    F --> G["Safety gate and external SDK bridge"]
```

## External robot boundaries

This repository stops at the vendor-neutral `/g1/upper_body_position_command` message. The following components must be supplied and independently validated in the target robot environment:

- a Unitree SDK2 bridge that communicates with the physical actuators;
- the exact G1/Dex3 URDF, SRDF, MoveIt groups, frames, and limits;
- a lower-body standing and balance controller;
- calibrated drivers and clocks for all three cameras;
- an XR sender that implements the documented UDP/JSON contract;
- physical emergency stops, collision zones, velocity/torque constraints, and a formal safety procedure.

No hardware-success claim is implied by the presence of these interfaces.
