...
def create_torch_dataset(
    ...
    if data_config.sidecar.enabled:
        dataset = _g1_training.SidecarDataset(
            dataset,
            _g1_training.read_sidecar(data_config.sidecar.path),
            include_text=data_config.sidecar.append_memory_to_prompt,
        )

    if data_config.action_valid_dim is not None:
        dataset = _g1_training.ActionMaskDataset(
            dataset,
            action_horizon=action_horizon,
            valid_action_dim=data_config.action_valid_dim,
        )
...
def transform_dataset(dataset: Dataset, data_config: _config.DataConfig, *, skip_norm_stats: bool = False) -> Dataset:
    ...
    return TransformedDataset(
        ...
        [
            ...
            *([_g1_training.AppendMemoryToPrompt()] if data_config.sidecar.append_memory_to_prompt else []),
    ...
    def __iter__(self):
        for batch in self._data_loader:
            ...
            for key in list(batch.keys()):
                if _g1_training.is_g1_key(key):
                    value = batch.pop(key)
                    if key in _g1_training.NUMERIC_METADATA_KEYS:
                        metadata[key] = value
