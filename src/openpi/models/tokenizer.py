...
class FASTTokenizer:
        ...
        *,
        ...
        if actions is not None:
            ...
            if actions.ndim != 2 or not np.isfinite(actions).all():
                raise ValueError("actions must be a finite two-dimensional [H,D] array")
        ...
        postfix_tokens = []
        if subtask:
            cleaned_subtask = str(subtask).strip().replace("_", " ").replace("\n", " ")
            postfix_tokens += self._paligemma_tokenizer.encode(f"Subtask: {cleaned_subtask};\n")
    ...
    def extract_subtask(self, tokens: np.ndarray) -> str | None:
        """Extract the optional autoregressively generated subtask string."""

        decoded = self._paligemma_tokenizer.decode(np.asarray(tokens, dtype=np.int32).tolist())
        if "Subtask: " not in decoded:
            return None
        value = decoded.split("Subtask: ", 1)[1]
        value = value.split("Action: ", 1)[0].split(";", 1)[0].strip()
