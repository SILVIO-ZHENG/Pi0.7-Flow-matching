...
try:
    ...
    if exc.name is not None and (exc.name == "lerobot" or exc.name.startswith("lerobot.")):
        # The main OpenPI environment pins an older LeRobot version. Provide only
        # the V3/V2.1 metadata helpers needed for conversion to avoid installing
        # the complete newer LeRobot and PyTorch stack.
...
def convert_episodes_metadata(new_root: Path, episode_records: list[dict[str, Any]]) -> None:
    ...
    with (
        ...
        for record in sorted(episode_records, key=lambda rec: int(rec["episode_index"])):
            ...
            # Recover length from the data index when an older episode lacks it.
